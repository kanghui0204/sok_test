    ...
    def __init__(
        ...
        **kwargs
        ...
        self.out_layer = tf.keras.layers.Dense(
            units=1, activation=None, kernel_initializer="ones", bias_initializer="zeros"
        )
...
class TfDenseDemo(tf.keras.models.Model):
    def __init__(
        ...
        **kwargs
        ...
        self.embedding_layer = TFDistributedEmbedding(
            vocabulary_size=self.vocabulary_size, embedding_vec_size=self.embedding_vec_size
        )
...
class SOKDenseModel(tf.keras.models.Model):
    def __init__(
        ...
        **kwargs
        ...
        if len(slot_num_list) != len(nnz_per_slot_list) or len(slot_num_list) != len(
            embedding_vec_size_list
        ):
            raise ValueError(
                "The length of embedding_vec_size_list, slot_num_list"
        ...
        for i in range(1, self.embedding_num + 1):
            self.slot_num_prefix_sum[i] = (
                self.slot_num_prefix_sum[i - 1] + self.slot_num_list[i - 1]
            )
        ...
        for _ in range(self.num_dense_layers):
            self.layer = tf.keras.layers.Dense(
                units=1024, activation="relu", kernel_initializer="ones", bias_initializer="zeros"
            )
            self.dense_layers.append(self.layer)
    ...
    def call(self, inputs, training=True):
        ...
        embedding_vector = self.do_lookup(
            self.embedding_layers[0],
        ...
        embedding_vector = tf.reshape(
            ...
            shape=[
                -1,
                self.slot_num_list[0] * self.nnz_per_slot_list[0] * self.embedding_vec_size_list[0],
            ],
        )
        ...
        for i in range(1, self.embedding_num):
            with tf.control_dependencies([embedding_vector]):
                ...
                embedding_vector = tf.reshape(
                    ...
                    shape=[
                        -1,
                        self.slot_num_list[i]
                        * self.nnz_per_slot_list[i]
                        * self.embedding_vec_size_list[i],
                    ],
                )
                vectors.append(embedding_vector)
...
class TFDenseModel(tf.keras.models.Model):
        ...
        **kwargs
        ...
        for i in range(self.embedding_num):
            self.param = self.add_weight(
                shape=(self.vocabulary_size, self.embedding_vec_size_list[i]),
                dtype=tf.float32,
                name="embedding_table_" + str(i),
    ...
    def call(self, inputs, training=True):
        ...
        embedding_vector = tf.nn.embedding_lookup(
            params=self.embedding_params[0],
        ...
        for i in range(1, self.embedding_num):
            with tf.control_dependencies([embedding_vector]):
                embedding_vector = tf.nn.embedding_lookup(
                    params=self.embedding_params[i],
